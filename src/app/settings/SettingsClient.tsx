...
export default function SettingsPage({ initialSettings, initialBathrooms, initialRooms }: SettingsPageProps) {
    const [settings] = useState<Record<string, SettingItem>>(initialSettings);
    const initVals: Record<string, string> = {};
    Object.entries(initialSettings).forEach(([key, val]) => { initVals[key] = val.value; });
    const [values, setValues] = useState<Record<string, string>>(initVals);
    const [saving, setSaving] = useState(false);
    const [toast, setToast] = useState<{ message: string; type: ToastType } | null>(null);

    // Bathroom management
    const [bathrooms, setBathrooms] = useState<Bathroom[]>(initialBathrooms);
    ...
    const fetchBathrooms = async () => {
        ...
        setBathrooms(Array.isArray(data) ? data : []);
    };
